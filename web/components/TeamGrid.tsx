...
export default function TeamGrid({ teams }: { teams: TeamMeta[] }) {
  const { t } = useI18n()
  const [query, setQuery] = useSessionState('teams:query', '')
  ...
  const filtered = useMemo(() => {
    ...
    return teams.filter(team =>
      ...
      (team.tags ?? []).some((tag: string) => tag.toLowerCase().includes(q)) ||
      (team.members ?? []).some((m: { slug: string }) => m.slug.toLowerCase().includes(q))
    )
  }, [teams, query])
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        {filtered.length === 0 ? (
          <div className="text-center py-24" style={{ color: 'var(--text-muted)' }}>
            ...
            <p className="text-lg">{t.noTeams(query)}</p>
            <button
              onClick={() => setQuery('')}
